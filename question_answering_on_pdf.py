...
from libs.embedding import EmbeddingClient
...
class DataService:
    ...
    def pdf_to_embeddings(self, pdf_path: str, chunk_length: int = 8000):
        ...
        res = client.batchCreateEmbeddings('text-embedding-v3', 10, chunks)
        # id: 块编号
        # text: 原始文本块
        # vector: 模型生成的嵌入
        return [
            {
                'id': f"{value['batch_number']}{value['payload'].index}",
                'vector': value['payload'].embedding,
                'text': chunks[value['batch_number'] + value['payload'].index]
            } for value in res
        ]
    ...
    def search_redis(
        ...
        embedded_query = client.createEmbeddings(input=user_query).data[0].embedding
        # Prepare the Query
...
def main():
    ...
    data = data_svc.pdf_to_embeddings('./assets/塞尔达传说_旷野之息_中文版完全攻略本.pdf')
